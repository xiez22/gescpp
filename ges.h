...
namespace ges {
    ...
    auto delete_node(int x, int y, const std::set<int>& H, const torch::Tensor& A) {
        ...
        std::vector<int> h_nx;
        std::set_intersection(H.begin(), H.end(), n_x.begin(), n_x.end(), std::back_inserter(h_nx));
    ...
    auto score_valid_delete_operators(int x,
        ...
        for (ull sub = 0; sub < total_valid; ++sub) {
            ...
            for (int i = 0; i < H0.size(); ++i)
                if (((1ull << i) & sub) == (1ull << i))
                    H.insert(H0[i]);

                // Check cond1
                auto cond_1 = cond_1_list[sub];
                std::set<int> na_yx_h;
                std::set_difference(na_yx.begin(), na_yx.end(), H.begin(), H.end(), std::inserter(na_yx_h, na_yx_h.end()));
                ...
                if (cond_1) {
                    ...
                    auto pa_y = utils::pa(y, A);
                    std::set<int> aux;
                    std::set_union(na_yx_h.begin(), na_yx_h.end(), pa_y.begin(), pa_y.end(), std::inserter(aux, aux.end()));
                    aux.insert(x);
                    auto old_score = cache.local_score(y, aux);
                    aux.erase(x);
                    auto new_score = cache.local_score(y, aux);
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
                ...
                if (debug > 1)
                    ...
                    << std::endl;
                // Get score
                auto&& [score, new_A, valid_cnt] = score_valid_insert_operators(
    ...
    auto backward_step(const torch::Tensor& A,
        ...
        for (int i=0;i<fro.size();++i) {
            if (debug>1) {
                std::cout << "Testing remove " << fro[i] << " to " << to[i] << std::endl;
            }
            // Get score
            auto &&[score, new_A, valid_cnt] = score_valid_delete_operators(
